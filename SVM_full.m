...
train_data1 = csvread( '(scr) Negative RF - Tones - downsample_1.csv' );
cols = size(train_data1,2);
P = randperm(cols);
train_data1 = train_data1(:,P);
train_data = train_data1(2001:5000,:);
train_data = train_data';
%output = csvread( 'Negative RF - all safety NPs.csv' );
output = train_data1(1,:);
output = output';
...
% X_train = X(rand_num(0.5:round(1*length(rand_num))),:);
% y_train = y(rand_num(0.5:round(1*length(rand_num))),:);
% 
% X_test = X(rand_num(round(0.5*length(rand_num))+1:end),:);
% y_test = y(rand_num(round(0.5*length(rand_num))+1:end),:);
...
uistack(h1,'bottom');
uistack(h2,'bottom');
legend([hgscatter;h_sv],{'No Response','Response','support vectors'})
